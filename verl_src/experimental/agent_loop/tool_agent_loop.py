...
class ToolAgentLoop(AgentLoopBase):
    ...
    async def _handle_processing_tools_state(self, agent_data: AgentData) -> AgentState:
        ...
        # Accumulate tool metrics into extra_fields so they reach the reward function
        for tool_index, (tool_response, tool_reward, tool_metrics) in enumerate(responses):
            ...
            if tool_metrics:
                for k, v in tool_metrics.items():
                    prev = agent_data.extra_fields.get(k, 0)
                    if isinstance(v, (int, float)) and not isinstance(v, bool):
                        agent_data.extra_fields[k] = prev + v
                    elif isinstance(v, str):
                        # Keep the first non-"none" exception type
                        if v != "none":
                            agent_data.extra_fields.setdefault(k, v)
