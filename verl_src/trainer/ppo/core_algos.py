...
def compute_grpo_outcome_advantage(
    ...
    valid_sample_mask: Optional[list[bool] | torch.Tensor] = None,
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Compute advantage for GRPO, with optional TrajectoryFilter support.
    ...
    Args:
        ...
        index: uid array for grouping
        epsilon: small value to avoid division by zero
        norm_adv_by_std_in_grpo: whether to scale by std (original GRPO)
        config: algorithm configuration object
        valid_sample_mask: ``True`` for trajectories that should participate
            in group mean/std. ``None`` = all trajectories are valid (original behaviour).
...
def compute_grpo_vectorized_outcome_advantage(
    ...
    """
    Vectorized GRPO with optional TrajectoryFilter support.

    When ``valid_sample_mask`` is provided, system-anomaly trajectories are
    excluded from group mean/std.  Groups with <2 valid trajectories are zeroed.
    """
    with torch.no_grad():
        scores = token_level_rewards.sum(dim=-1)
        ...
        if valid_sample_mask is not None:
            ...
            for ug in unique_groups:
                group_mask = (g == ug)
                valid_in_group = mask & group_mask
                n_valid = valid_in_group.sum().item()
                if n_valid < 2:
                    # Zero out this entire group
                    scores[group_mask] = 0.0
                    continue
                # Compute mean/std using only valid samples in this group
                valid_scores = scores[valid_in_group]
                grp_mean = valid_scores.mean()
                grp_std = valid_scores.std()
                # Normalise only valid samples; set invalid to 0
                if norm_adv_by_std_in_grpo:
                    scores[valid_in_group] = (valid_scores - grp_mean) / (grp_std + epsilon)
                else:
                    scores[valid_in_group] = valid_scores - grp_mean
                scores[group_mask & ~mask] = 0.0
            advantages = scores.unsqueeze(-1) * response_mask
            return advantages, advantages

        # --- Original path (no mask) ---
